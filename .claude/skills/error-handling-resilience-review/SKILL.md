...
description: "Perform deep, production-grade error handling and resilience reviews using a multi-agent architecture where specialized sub-agents independently explore their domain in parallel and report findings back to a master orchestrator agent that synthesizes a unified, authoritative resilience report. Use this skill whenever reviewing code for error handling quality, resilience patterns, information leakage, retry/circuit breaker strategies, dead letter queue coverage, observability gaps, or production error readiness. Trigger on: error handling reviews, resilience audits, retry logic reviews, circuit breaker assessments, DLQ coverage, error response consistency, information leakage detection, observability audits, structured logging reviews, error tracking setup, distributed tracing reviews, async error handling, message queue error patterns, or any task where deep error handling and resilience analysis is needed — even if the user doesn't explicitly say 'error handling review'. Apply this skill even when the user asks to 'check my error handling', 'review my resilience', 'are errors handled properly', 'is this safe to deploy', 'will this fail gracefully', 'audit my error responses', or 'check for information leakage'."
---
