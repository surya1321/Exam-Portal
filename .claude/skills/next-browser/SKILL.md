...
`next-browser --version` and compare against the latest on pnpm
(`pnpm view @vercel/next-browser version`). If the installed version is
behind, upgrade it (`pnpm install -g @vercel/next-browser@latest` or the
