...
**Key findings:**
- Chrome installation takes ~24s (system deps + pnpm package + binary extraction)
...
| Chrome pnpm install           | ~15s   | `@sparticuz/chromium` + `puppeteer-core`     |
...
cd prototypes/cloud
pnpm install
pnpm run proto:sandbox    # Prototype 1
pnpm run proto:chrome     # Prototype 2
pnpm run proto:agent      # Prototype 3
pnpm run proto:full       # Prototype 4
```
